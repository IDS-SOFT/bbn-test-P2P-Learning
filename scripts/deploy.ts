...
async function main() {
  ...
  const P2PLearning = await hre.ethers.getContractFactory("P2PLearning");
  const p2PLearning = await P2PLearning.deploy();

  await p2PLearning.deployed();

  console.log("P2PLearning contract deployed to:", p2PLearning.address);

  // Store the contract addresses for later use
  const addresses = {
    p2PLearning: p2PLearning.address,
  };
